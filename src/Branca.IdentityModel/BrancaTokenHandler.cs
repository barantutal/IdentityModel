...
using Branca.IdentityModel.Crypto;
using Branca.IdentityModel.Encoding;
using Branca.IdentityModel.Extensions;
...
namespace Branca.IdentityModel.Branca
{
    public class BrancaTokenHandler : JwtPayloadTokenHandler
    {
        ...
        public virtual string CreateToken(string payload, uint timestamp, byte[] key)
            => CreateToken(System.Text.Encoding.UTF8.GetBytes(payload), timestamp, key);
